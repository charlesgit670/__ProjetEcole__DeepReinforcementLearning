...
class Environment():
        ...
        for comb in self.combinations:
            ...
            for c in comb:
                ...
                if (len(ladder[-1]) == 0) and (self.position_in_ladder_in_flight(ladderindex) < len(ladder)-1):
                    ...
                    if (self.position_in_ladder(self.current_player, ladderindex) > -1 or self.position_in_ladder_in_flight(ladderindex) > -1) and len(self.in_flight) < 3:
    ...
    def take_action(self, action):
        ...
        if self.phase == 0:
            ...
            self.actions = [
                ("Relancer les dés", self.ACTION_THROW_AGAIN),
                ("Sauver les états", self.ACTION_STICK_DOWN)
            ]
    ...
    def get_board_state(self):
        ...
        for row in self.board:
            ...
            #     elif cell == 'player2':
            #         row_state.append(2)
                # Add more conditions as necessary for other types of cells or pieces
    ...
    def render(self, colorize):
        ...
        print("< Tour #{current_turn} >".format(current_turn = self.current_turn).center(pagewidth, "-"))
        print()

        # Print last action that happened leading up to the board currently being displayed
        if (self.last_action):
            print((self.PLAYER_INFO[self.last_player][1] if colorize else "") + "Le joueur #{last_player} ({name}) a joué: {last_action}.".format(
            ...
            for ladder in range(0, len(self.board)):
                ...
                if (logicalrow == len(self.board[ladder])) or (logicalrow == -1):
                    ...
                    if self.position_in_ladder_in_flight(ladder) == logicalrow:
                        pieces += (colorama.Fore.YELLOW if colorize else "") + "●" + colorama.Fore.WHITE# "◌"
                        add_in_flight = 1
        ...
        if (self.dice):
            print("Resultat dés: " + str(self.dice))
        else:
            print()
